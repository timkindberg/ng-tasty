...
var $__2 = ($__dom_95_adapter__ = require("./dom_adapter"), $__dom_95_adapter__ && $__dom_95_adapter__.__esModule && $__dom_95_adapter__ || {default: $__dom_95_adapter__}),
...
($traceurRuntime.createClass)(Parse5DomAdapter, {
  ...
  querySelectorAll: function(el, selector) {
    ...
    var _recursive = (function(result, node, selector, matcher) {
      if ($__5.elementMatches(node, selector, matcher)) {
  ...
  elementMatches: function(node, selector) {
    ...
    assert.argumentTypes(node, assert.type.any, selector, assert.type.string, matcher, assert.type.any);
    var result = false;
  ...
  isCommentNode: function(node) {
    return assert.returnType((treeAdapter.isCommentNode(node)), assert.type.boolean);
  },
